...
impl Markov {
    ...
    fn add_string(&mut self, sentence: &str) {
    ...
    fn generate(&self, words: i16) -> String {
...
fn main() {
    let mut rust_hace_el_favor_de_compilar_strings_cohone = Markov::new();
    let lines = std::io::stdin().lines();
    for line in lines {
        rust_hace_el_favor_de_compilar_strings_cohone.add_string(&line.unwrap());
    }
    println!(
        "{}",
        rust_hace_el_favor_de_compilar_strings_cohone.generate(10)
    );
}
